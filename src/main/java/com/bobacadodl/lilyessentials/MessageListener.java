...
public class MessageListener 
{
	...
	public void onMessage(MessageEvent event) 
	{
		...
		if (event.getChannel().equalsIgnoreCase("lilyessentials.shout")) 
		{
			try 
			{
		...
		if (event.getChannel().equalsIgnoreCase("lilyessentials.global")) 
		{
			try 
			{
				String code = event.getMessageAsString();
				...
				String chat = ChatColor.translateAlternateColorCodes(
						'&',
						plugin.getCfg().format_global.replace("{player}", user)
						.replace("{message}", message)
						.replace("{server}", plugin.getUsername()));
